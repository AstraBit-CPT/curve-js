...
async function all(provider, multicall, calls, overrides) {
    const contract = multicall
        ? new ethers_1.Contract(multicall.address, multicall_json_1.default, provider)
        : null;
    const callRequests = calls.map((call) => {
        const callData = abi_js_1.default.encode(call.name, call.inputs, call.params);
        return {
    ...
    for (let i = 0; i < callCount; i++) {
        ...
        const params = abi_js_1.default.decode(name, outputs, returnData);
...
async function tryAll(provider, multicall2, calls, overrides) {
    const contract = multicall2
        ? new ethers_1.Contract(multicall2.address, multicall2_json_1.default, provider)
        : null;
    const callRequests = calls.map((call) => {
        const callData = abi_js_1.default.encode(call.name, call.inputs, call.params);
        return {
    ...
    for (let i = 0; i < callCount; i++) {
        ...
        if (!result.success) {
            ...
            try {
                const params = abi_js_1.default.decode(name, outputs, result.returnData);
...
async function tryEach(provider, multicall3, calls, overrides) {
    const contract = multicall3
        ? new ethers_1.Contract(multicall3.address, multicall3_json_1.default, provider)
        : null;
    const callRequests = calls.map((call) => {
        const callData = abi_js_1.default.encode(call.name, call.inputs, call.params);
        return {
...
async function callDeployless(provider, callRequests, block) {
    ...
    const args = abi_js_1.default.encodeConstructor(inputs, [callRequests]);
    const data = (0, ethers_1.concat)([multicall_js_1.deploylessMulticallBytecode, args]);
    ...
    const response = abi_js_1.default.decode(name, outputs, callData);
    return response;
}
async function callDeployless2(provider, callRequests, block) {
    ...
    const constructor = inputAbi.find((f) => f.type === 'constructor');
    const inputs = constructor?.inputs || [];
    const args = abi_js_1.default.encodeConstructor(inputs, [false, callRequests]);
    const data = (0, ethers_1.concat)([multicall_js_1.deploylessMulticall2Bytecode, args]);
    const callData = await provider.call({
        data,
        blockTag: block,
    });
    ...
    const response = abi_js_1.default.decode(name, outputs, callData)[0];
    return response;
}
async function callDeployless3(provider, callRequests, block) {
    ...
    const inputs = constructor?.inputs || [];
    const args = abi_js_1.default.encodeConstructor(inputs, [callRequests]);
    const data = (0, ethers_1.concat)([multicall_js_1.deploylessMulticall3Bytecode, args]);
    const callData = await provider.call({
        data,
        blockTag: block,
    });
